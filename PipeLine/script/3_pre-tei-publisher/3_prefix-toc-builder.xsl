...
<xsl:stylesheet
    ...
    <!--
        Calculé à partir du xml:id de l'élément racine <TEI> du document
        source, qui suit toujours le schéma :
            <NomAuteur>_<RéférenceBiblique>_<Reste>
        ex. xml:id="Anonymus_1-Tim_C1-6"
            -> auteur   = "Anonymus" (segment 1)
            -> référence = "1-Tim"    (segment 2)
    ...
    <xsl:param name="prefix" as="xs:string" select="
        let $rootId := /*/@xml:id/string(),
            $parts  := tokenize($rootId, '_'),
            $author3 := substring($parts[1], 1, 3),
            $bibleRef := $parts[2]
        return concat($author3, '_', $bibleRef)
    "/>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <div type="book-introduction"
                                xml:id="{concat($prefix, '_Introduction')}">
