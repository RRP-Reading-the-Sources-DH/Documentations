...
<!-- Source XSL : Simon Gabay ; adaptation : Sonia Solfrini ; adaptation Floriane Goy-->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Les variables permettent de reconstituer l'url des documents et par la suite l'adresse iif du document, lors organisations varient en fonction de 2 éléments : 
    1. le système de nommage des fichiers,
    2. la structure du manifest iiif qui varie d'un dépot à l'autre.
    /!\ N'oublier pas de commenter/ dec0mmenter la déclaration des variables qui correspond à votre type de manifest iiif ici , e-rara ; mdz ou SBB (berlin),
     les documents de la ONB ont une structure différent qui requiert un autre script =>  1_alto2sourceDoc_ONB.xsl 
     -->
    ...
    <xsl:variable name="document">Megander_Eph_C1-6</xsl:variable>
    <xsl:variable name="folderName">doc_1</xsl:variable>
    <xsl:variable name="iiif-domain">iiif:</xsl:variable>
    ...
    <!-- for Staatsbibliothek zu Berlin (SBB) -->
    
    <!--
    <xsl:variable name="document">Melanchthon_2-Cor_C1-11</xsl:variable>
    <xsl:variable name="folderName">doc_1</xsl:variable>
    <xsl:variable name="iiif-domain">iiif:</xsl:variable>
    <xsl:variable name="serveur">https://content.staatsbibliothek-berlin.de/</xsl:variable>
    <xsl:variable name="iiif-name">dc/</xsl:variable>
    <xsl:variable name="endfile">/full/full/0/default.jpg</xsl:variable>
    -->
